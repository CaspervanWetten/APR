...
class PromptingEngine:
    ...
    def __init__(self, api, templates_path: str):
        ...
        with open(templates_path, 'r', encoding='utf-8') as f:
            self.templates = json.load(f)  # Loads the prompt templates from the file.
    ...
    def generate_prompt(self, template_name: str, **kwargs) -> tuple[str, str]:
        ...
        """
        template = self.templates.get(template_name)  # Retrieves the template based on the name.
        if not template:
            raise KeyError(f"Template '{template_name}' not found in templates.")  # Raises an error if the template is missing.

        system_prompt = template.get('system', '').format(**kwargs)  # Fills the system prompt template with variables.
        user_prompt = template.get('user', '').format(**kwargs)  # Fills the user prompt template with variables.
    ...
    def generate_response(self, template_name, **kwargs):
        ...
        """
        system_prompt, user_prompt = self.generate_prompt(template_name, **kwargs)
        ...
        match model:
            ...
            case "claude-3-7-sonnet-20250219":
                res = self._generate_anthropic(system_prompt, user_prompt, model)
            #Cintiqo models
            case "QoPilot-1":
    ...
    def _generate_openAI(self, system_prompt, user_prompt, model):
        ...
        if not self.openAI_key:
            raise NotImplementedError("OpenAI API key not provided.")  # Raises an error if the OpenAI key is not set.
        
        client = OpenAI(api_key=self.openAI_key, timeout=120.0, max_retries=3)  # Initializes the OpenAI client with the API key.
        
        messages = []  # Initializes a list to hold the conversation messages.
        if system_prompt:
            messages.append({"role": "system", "content": system_prompt})  # Adds the system prompt if it exists.
        messages.append({"role": "user", "content": user_prompt})  # Adds the user prompt.

        response = client.chat.completions.create(  # Makes the API call to OpenAI to generate a completion.
            model=model,
            messages=messages
        )
        return response.choices[0].message.content  # Returns the content of the first response choice.
    ...
    def _generate_anthropic(self, system_prompt, user_prompt, model):
        ...
        if not self.anthropic_key:
            raise NotImplementedError("Claude API key not provided.")  # Raises an error if the Claude key is not set.
        ...
        return response.content[0].text  # Return the text content from the response
